...
static void key_pressure_horizontal_impl(uint16_t* fb, ui_layer_t* layer, float bar_length, int rect_x, int bar_x)
{
    layer->x += 14;
    int rect_w = 14;
    int rect_h = 14;
    ...
    {
        draw_rect(fb, layer->x + rect_x, layer->y, rect_w, rect_h, bg_color);
    }
    draw_horizontal_line(fb, layer->x + rect_x, layer->y, rect_w, layer->color);
    draw_horizontal_line(fb, layer->x + rect_x, layer->y + rect_h - line_width, rect_w, layer->color);
    draw_vertical_line(fb, layer->x + rect_x, layer->y, rect_h, layer->color);
    draw_vertical_line(fb, layer->x + rect_x + rect_w - line_width, layer->y, rect_h, layer->color);   
}

static void key_pressure_horizontal(uint16_t* fb, ui_layer_t* layer)
{
    return key_pressure_horizontal_impl(fb, layer, 1.27f, -14, 4);
}

static void key_pressure_horizontal_inverse(uint16_t* fb, ui_layer_t* layer)
{
    return key_pressure_horizontal_impl(fb, layer, 1.15f, 80, -12);
}
...
void custom_widget_handler(uint16_t* fb, ui_layer_t* layer)
{
    switch (layer->material)
    {
        case 16 - 1:
            ...
            break;
        case 17 - 1:
            key_pressure_horizontal_inverse(fb, layer);
            break;
        case 18 - 1:
